...
import tw.openedu.android.base.CourseDetailBaseFragment;
...
import tw.openedu.android.social.facebook.FacebookProvider;
import tw.openedu.android.task.GetAnnouncementTask;
import tw.openedu.android.util.StandardCharsets;
import tw.openedu.android.util.WebViewUtil;
import tw.openedu.android.view.custom.EdxWebView;
import tw.openedu.android.view.custom.URLInterceptorWebViewClient;
...
public class CourseCombinedInfoFragment extends CourseDetailBaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(tw.openedu.android.R.layout.fragment_course_combined_info, container, false);

        announcementWebView = (EdxWebView) view.findViewById(tw.openedu.android.R.id.announcement_webview);
        ...
        notificationSettingRow = view.findViewById(tw.openedu.android.R.id.notificaton_setting_row);
        notificationSwitch = (Switch) view.findViewById(tw.openedu.android.R.id.notification_switch);
    ...
    private void loadAnnouncementData(EnrolledCoursesResponse enrollment) {
        ...
        ProgressBar progressBar = (ProgressBar) getView().findViewById(tw.openedu.android.R.id.loading_indicator);
    ...
    public void showEmptyAnnouncementMessage() {
        try {
            if (getView() != null) {
                getView().findViewById(tw.openedu.android.R.id.no_announcement_tv).setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
            logger.error(e);
        }
    ...
    private void hideEmptyAnnouncementMessage() {
        try {
            if (getView() != null) {
                getView().findViewById(tw.openedu.android.R.id.no_announcement_tv).setVisibility(View.GONE);
            }
        } catch (Exception e) {
            logger.error(e);
        }
