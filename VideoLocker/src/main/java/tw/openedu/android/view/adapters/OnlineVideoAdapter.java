...
import tw.openedu.android.model.api.ChapterModel;
import tw.openedu.android.model.api.SectionItemModel;
...
public abstract class OnlineVideoAdapter extends VideoBaseAdapter<SectionItemInterface> {
    ...
    public OnlineVideoAdapter(Context context,  IEdxEnvironment environment) {
        super(context, tw.openedu.android.R.layout.row_video_list, environment);
                ...
                dbStore.getWatchedStateForVideoId(videoData.videoId, 
                    ...
                    public void onResult(DownloadEntry.WatchedState result) {
                        ...
                        if(ws == null || ws == DownloadEntry.WatchedState.UNWATCHED) {
                            holder.video_watched_status.setImageResource(tw.openedu.android.R.drawable.cyan_circle);
                        } else if(ws == DownloadEntry.WatchedState.PARTIALLY_WATCHED) {
                            holder.video_watched_status.setImageResource(tw.openedu.android.R.drawable.ic_partially_watched);
                        } else {
                            holder.video_watched_status.setImageResource(tw.openedu.android.R.drawable.grey_circle);
                        }
                    }
                    @Override
                    ...
                    public void onResult(DownloadEntry.DownloadedState result) {
                        ...
                        if(selectedVideoId!=null){
                            if (selectedVideoId.equalsIgnoreCase(videoData.videoId)) {
                                ...
                                holder.videolayout.setBackgroundResource(tw.openedu.android.R.color.cyan_text_navigation_20);
                                if(isPlayerOn){
                                    holder.video_download_layout.setVisibility(View.GONE);
                                }
                            } else {
                                // mark this cell as non-selected
                                holder.videolayout.setBackgroundResource(tw.openedu.android.R.drawable.list_item_overlay_selector);
                            }
                        }else{
                            holder.videolayout.setBackgroundResource(tw.openedu.android.R.drawable.list_item_overlay_selector);
                        }
    ...
    public BaseViewHolder getTag(View convertView) {
        ...
        holder.video_download_layout = (LinearLayout) convertView
                .findViewById(tw.openedu.android.R.id.video_download_layout);
        holder.download_pw = (ProgressWheel) convertView
                .findViewById(tw.openedu.android.R.id.progress_wheel);
        holder.videoTitle = (TextView) convertView
                .findViewById(tw.openedu.android.R.id.video_title);
        holder.videoPlayingTime = (TextView) convertView
                .findViewById(tw.openedu.android.R.id.video_playing_time);
        holder.videoSize = (TextView) convertView
                .findViewById(tw.openedu.android.R.id.video_size);
        holder.video_watched_status = (ImageView) convertView
                .findViewById(tw.openedu.android.R.id.video_watched_status);
        holder.course_title = (TextView) convertView
                .findViewById(tw.openedu.android.R.id.txt_course_title);
        holder.section_title = (TextView) convertView
                .findViewById(tw.openedu.android.R.id.txt_chapter_title);
        holder.videolayout = (RelativeLayout) convertView
                .findViewById(tw.openedu.android.R.id.video_row_layout);
        holder.progresslayout = (LinearLayout) convertView
                .findViewById(tw.openedu.android.R.id.download_progress);
        return holder;
    }
