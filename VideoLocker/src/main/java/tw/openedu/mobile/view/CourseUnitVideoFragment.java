...
import tw.openedu.mobile.model.api.LectureModel;
...
import tw.openedu.mobile.player.IPlayerEventCallback;
import tw.openedu.mobile.player.PlayerFragment;
import tw.openedu.mobile.player.TranscriptManager;
import tw.openedu.mobile.services.ViewPagerDownloadManager;
import tw.openedu.mobile.task.CircularProgressTask;
import tw.openedu.mobile.util.AppConstants;
...
import tw.openedu.mobile.view.dialog.DeleteVideoDialogFragment;
...
public class CourseUnitVideoFragment extends CourseUnitFragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(tw.openedu.mobile.R.layout.fragment_course_unit_video, container, false);
        messageContainer = v.findViewById(tw.openedu.mobile.R.id.message_container);
        return v;
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (playerFragment == null) {
            ...
            try{
                ...
                ft.replace(tw.openedu.mobile.R.id.player_container, playerFragment, "player");
                ft.commit();
            }catch(Exception ex){
                logger.error(ex);
            }
    ...
    private boolean checkDownloadEntry(DownloadEntry entry) {
        ...
        if (entry.isVideoForWebOnly) {
            Toast.makeText(getContext(), getString(tw.openedu.mobile.R.string.video_only_on_web_short),
        ...
        if (checkDownloadEntry(entry) && !entry.isDownloaded()) {
            if (!MediaConsentUtils.canStreamMedia(getContext())) {
                ((BaseFragmentActivity) getActivity()).
                        showInfoMessage(getString(tw.openedu.mobile.R.string.wifi_off_message));
            }
        }
    }
    ...
    private void startOnlineDownload(DownloadEntry videoData, ProgressWheel progressWheel){
        ...
        if (downloadSize > MemoryUtil
            ...
            ((BaseFragmentActivity) getActivity())
                .showInfoMessage(getString(tw.openedu.mobile.R.string.file_size_exceeded));
            notifyAdapter();
        } else {
    ...
    private void showPlayer() {
        ...
        try {
            if(getView()!=null){
                View container = getView().findViewById(tw.openedu.mobile.R.id.player_container);
                if (container != null) {
    ...
    private void hideOpenInBrowserPanel() {
        try {
            getView().findViewById(tw.openedu.mobile.R.id.open_in_browser_panel).setVisibility(
                View.GONE);
        } catch (Exception ex) {
            logger.error(ex);
        ...
        try {
            if (NetworkUtil.isConnected(getActivity())) {
                if (isPlayerVisible()) {
                    ...
                    getView().findViewById(tw.openedu.mobile.R.id.open_in_browser_panel)
                        .setVisibility(View.VISIBLE);
                    TextView openInBrowserTv = (TextView) getView().findViewById(
                        tw.openedu.mobile.R.id.open_in_browser_btn);
...
//            }
    ...
    public void startDownload(final DownloadEntry downloadEntry, final ProgressWheel progressWheel) {
        try{
                ...
                if (environment.getStorage().addDownload(downloadEntry) != -1) {
                    ((BaseFragmentActivity) getActivity())
                        .showInfoMessage(getString(tw.openedu.mobile.R.string.msg_started_one_video_download));
                } else {
                    ((BaseFragmentActivity) getActivity())
                        .showInfoMessage(getString(tw.openedu.mobile.R.string.msg_video_not_downloaded));
                }
    ...
    protected void showStartDownloadDialog(final DownloadEntry de, final ProgressWheel progressWheel) {
        ...
        dialogMap.put("title", getString(tw.openedu.mobile.R.string.download_exceed_title));
        dialogMap.put("message_1", getString(tw.openedu.mobile.R.string.download_exceed_message));
        dialogMap.put("yes_button", getString(tw.openedu.mobile.R.string.label_yes));
        dialogMap.put("no_button",  getString(tw.openedu.mobile.R.string.label_no));
    ...
    private boolean isPlayerVisible() {
        ...
        View container = getActivity().findViewById(tw.openedu.mobile.R.id.container_player);
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            LinearLayout playerContainer = (LinearLayout)getView().findViewById(tw.openedu.mobile.R.id.player_container);
