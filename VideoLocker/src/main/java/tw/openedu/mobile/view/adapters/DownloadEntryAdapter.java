...
public abstract class DownloadEntryAdapter extends BaseListAdapter<DownloadEntryAdapter.Item> {
    ...
    public DownloadEntryAdapter(Context context, IEdxEnvironment environment) {
        super(context, tw.openedu.mobile.R.layout.row_download_list, environment);
    }
    ...
    public void render(BaseViewHolder tag, final Item item) {
        ...
        switch (item.getStatus()) {
            case PENDING: {
                progressText = getContext().getString(tw.openedu.mobile.R.string.download_pending);
                progressDrawable = tw.openedu.mobile.R.drawable.custom_progress_bar_horizontal_green;
                errorText = null;
                break;
            }
            case DOWNLOADING: {
            ...
            case FAILED: {
                errorText = getContext().getString(tw.openedu.mobile.R.string.error_download_failed);
                progressDrawable = tw.openedu.mobile.R.drawable.custom_progress_bar_horizontal_red;
    ...
    private static class ViewHolder extends BaseViewHolder {
        ...
        public ViewHolder(@NonNull View view) {
            title = (TextView) view.findViewById(tw.openedu.mobile.R.id.downloads_name);
            duration = (TextView) view
                    .findViewById(tw.openedu.mobile.R.id.download_time);
            percent = (TextView) view
                    .findViewById(tw.openedu.mobile.R.id.download_percentage);
            error = (TextView) view
                    .findViewById(tw.openedu.mobile.R.id.txtDownloadFailed);
            progress = (ProgressBar) view
                    .findViewById(tw.openedu.mobile.R.id.progressBar);
            cross_image_layout = (LinearLayout) view
                    .findViewById(tw.openedu.mobile.R.id.close_btn_layout);
        }
    }
