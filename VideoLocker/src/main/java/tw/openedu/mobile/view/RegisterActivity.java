...
import tw.openedu.mobile.exception.LoginException;
import tw.openedu.mobile.model.api.AuthResponse;
import tw.openedu.mobile.model.api.FormFieldMessageBody;
import tw.openedu.mobile.model.api.ProfileModel;
import tw.openedu.mobile.model.api.RegisterResponse;
import tw.openedu.mobile.model.api.RegisterResponseFieldError;
import tw.openedu.mobile.module.analytics.ISegment;
import tw.openedu.mobile.module.prefs.PrefManager;
import tw.openedu.mobile.module.registration.model.RegistrationAgreement;
import tw.openedu.mobile.module.registration.model.RegistrationDescription;
import tw.openedu.mobile.module.registration.model.RegistrationFieldType;
import tw.openedu.mobile.module.registration.model.RegistrationFormField;
import tw.openedu.mobile.module.registration.view.IRegistrationFieldView;
import tw.openedu.mobile.social.SocialFactory;
import tw.openedu.mobile.social.SocialLoginDelegate;
import tw.openedu.mobile.task.RegisterTask;
import tw.openedu.mobile.task.Task;
import tw.openedu.mobile.util.NetworkUtil;
import tw.openedu.mobile.util.ResourceUtil;
...
public class RegisterActivity extends BaseFragmentActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(tw.openedu.mobile.R.layout.activity_register);
        ...
        if (!isSocialEnabled) {
            findViewById(tw.openedu.mobile.R.id.panel_social_layout).setVisibility(View.GONE);
            findViewById(tw.openedu.mobile.R.id.or_signup_with_email_title).setVisibility(View.GONE);
            findViewById(tw.openedu.mobile.R.id.signup_with_row).setVisibility(View.GONE);
        } else {
            ImageView imgFacebook = (ImageView) findViewById(tw.openedu.mobile.R.id.img_facebook);
            ImageView imgGoogle = (ImageView) findViewById(tw.openedu.mobile.R.id.img_google);

            if (!environment.getConfig().getFacebookConfig().isEnabled()) {
                findViewById(tw.openedu.mobile.R.id.img_facebook).setVisibility(View.GONE);
            } else {
            ...
            if (!environment.getConfig().getGoogleConfig().isEnabled()) {
                findViewById(tw.openedu.mobile.R.id.img_google).setVisibility(View.GONE);
            } else {
        ...
        TextView agreementMessageView = (TextView) findViewById(tw.openedu.mobile.R.id.by_creating_account_tv);
        CharSequence agreementMessage = ResourceUtil.getFormattedString(getResources(), tw.openedu.mobile.R.string.by_creating_account, "platform_name", environment.getConfig().getPlatformName());
        agreementMessageView.setText(agreementMessage);

        createAccountBtn = (ViewGroup) findViewById(tw.openedu.mobile.R.id.createAccount_button_layout);
        ...
        createAccountTv = (TextView) findViewById(tw.openedu.mobile.R.id.create_account_tv);
        requiredFieldsLayout = (LinearLayout) findViewById(tw.openedu.mobile.R.id.required_fields_layout);
        optionalFieldsLayout = (LinearLayout) findViewById(tw.openedu.mobile.R.id.optional_fields_layout);
        agreementLayout = (LinearLayout) findViewById(tw.openedu.mobile.R.id.layout_agreement);
        final TextView optional_text = (TextView) findViewById(tw.openedu.mobile.R.id.optional_field_tv);
        optional_text.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (optionalFieldsLayout.getVisibility() == View.VISIBLE) {
                    ...
                    optional_text.setText(getString(tw.openedu.mobile.R.string.show_optional_text));
                } else {
                    optionalFieldsLayout.setVisibility(v.VISIBLE);
                    optional_text.setText(getString(tw.openedu.mobile.R.string.hide_optional_text));
                }
            }
        });

        View closeButton = findViewById(tw.openedu.mobile.R.id.actionbar_close_btn);
        if (closeButton != null) {
        ...
        registrationLayout = (LinearLayout) findViewById(tw.openedu.mobile.R.id.registrationLayout);

        TextView customTitle = (TextView) findViewById(tw.openedu.mobile.R.id.activity_title);
        if (customTitle != null) {
            CharSequence title = ResourceUtil.getFormattedString(getResources(), tw.openedu.mobile.R.string.register_title, "platform_name", environment.getConfig().getPlatformName());
    ...
    public void showAgreement(RegistrationAgreement agreement) {
        ...
        if (isInAppEULALink) {
            ...
            environment.getRouter().showWebViewDialog(this, getString(tw.openedu.mobile.R.string.eula_file_link), getString(tw.openedu.mobile.R.string.end_user_title));
        } else {
    ...
    private void createAccount() {
        if (NetworkUtil.isConnected(this)) {
            ScrollView scrollView = (ScrollView) findViewById(tw.openedu.mobile.R.id.scrollview);
            ...
            try {
                ...
                String appVersion = String.format("%s v%s", getString(tw.openedu.mobile.R.string.android), versionName);
                        ...
                        if (!result.isSuccess()) {
                            ...
                            if (messageBody == null || messageBody.isEmpty()) {
                                ...
                                if (errorMessage == null || errorMessage.isEmpty()) {
                                    errorMessage = getString(tw.openedu.mobile.R.string.sign_up_error);
                                }
                            ...
                            if (auth != null && auth.isSuccess()) {
                                ...
                                RegisterActivity.this.showErrorMessage(null, getString(tw.openedu.mobile.R.string.sign_up_error));
                            }
                        }
                    } else {
                        hideProgress();
            ...
            RegisterActivity.this.showErrorMessage(getString(tw.openedu.mobile.R.string.no_connectivity), getString(tw.openedu.mobile.R.string.network_not_connected));
        }
    }
    ...
    private void showRegularMessage(SocialFactory.SOCIAL_SOURCE_TYPE socialType) {
        LinearLayout messageLayout = (LinearLayout) findViewById(tw.openedu.mobile.R.id.message_layout);
        TextView messageView = (TextView) findViewById(tw.openedu.mobile.R.id.message_body);
        ...
        if (socialType == SocialFactory.SOCIAL_SOURCE_TYPE.TYPE_FACEBOOK) {
            socialTypeString = getString(tw.openedu.mobile.R.string.facebook_text);
            signUpSuccessString = getString(tw.openedu.mobile.R.string.sign_up_with_facebook_ok);
        } else {  //google
            socialTypeString = getString(tw.openedu.mobile.R.string.google_text);
            signUpSuccessString = getString(tw.openedu.mobile.R.string.sign_up_with_google_ok);
        }
        StringBuilder sb = new StringBuilder();
        CharSequence extraInfoPrompt = ResourceUtil.getFormattedString(getResources(), tw.openedu.mobile.R.string.sign_up_with_social_ok, "platform_name", environment.getConfig().getPlatformName());
    ...
    private void updateUIOnSocialLoginToEdxFailure(SocialFactory.SOCIAL_SOURCE_TYPE socialType, String accessToken) {
        //change UI.
        View signupWith = findViewById(tw.openedu.mobile.R.id.signup_with_row);
        signupWith.setVisibility(View.GONE);
        View socialPanel = findViewById(tw.openedu.mobile.R.id.panel_social_layout);
        socialPanel.setVisibility(View.GONE);
        TextView signupWithEmailTitle = (TextView) findViewById(tw.openedu.mobile.R.id.or_signup_with_email_title);
        signupWithEmailTitle.setText(getString(tw.openedu.mobile.R.string.complete_registration));
        //help method
    ...
    public boolean showErrorMessage(String header, String message, boolean isPersistent) {
        if (message != null) {
            ...
            return super.showErrorMessage(header, getString(tw.openedu.mobile.R.string.login_failed), isPersistent);
        }
    }
...
//        }
    ...
    private void showProgress() {
        ...
        View progress = findViewById(tw.openedu.mobile.R.id.progress_indicator);
        progress.setVisibility(View.VISIBLE);
        createAccountTv.setText(getString(tw.openedu.mobile.R.string.creating_account_text));
    }
    ...
    private void hideProgress() {
        ...
        createAccountTv.setText(getString(tw.openedu.mobile.R.string.create_account_text));
    }


    //Disable the Create button during server call
    private void createButtonDisabled() {
        createAccountBtn.setBackgroundResource(tw.openedu.mobile.R.drawable.new_bt_signin_active);
    ...
    private void createButtonEnabled() {
        createAccountBtn.setBackgroundResource(tw.openedu.mobile.R.drawable.bt_signin_active);
