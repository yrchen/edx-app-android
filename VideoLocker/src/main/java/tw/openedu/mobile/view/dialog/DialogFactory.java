...
public class DialogFactory {
    ...
    public static Dialog getChargesApplyConfirmationDialog(final FragmentActivity activity, final String uri) {
        ...
        dialog.setTitle(activity.getString(tw.openedu.mobile.R.string.open_external_url_title));
        dialog.setMessage(activity.getString(tw.openedu.mobile.R.string.open_external_url_desc));
        dialog.setPositiveButton(tw.openedu.mobile.R.string.label_continue, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface d, int which) {
                BrowserUtil.open(activity, uri);
                d.dismiss();
            }
        });
        dialog.setNegativeButton(tw.openedu.mobile.R.string.label_cancel, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface d, int which) {
                // do nothing
                d.dismiss();
