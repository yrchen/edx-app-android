...
import tw.openedu.mobile.base.CourseDetailBaseFragment;
import tw.openedu.mobile.model.api.AnnouncementsModel;
import tw.openedu.mobile.model.api.EnrolledCoursesResponse;
import tw.openedu.mobile.module.facebook.IUiLifecycleHelper;
import tw.openedu.mobile.social.facebook.FacebookProvider;
import tw.openedu.mobile.task.GetAnnouncementTask;
import tw.openedu.mobile.util.StandardCharsets;
import tw.openedu.mobile.util.WebViewUtil;
import tw.openedu.mobile.view.custom.EdxWebView;
import tw.openedu.mobile.view.custom.URLInterceptorWebViewClient;
...
public class CourseCombinedInfoFragment extends CourseDetailBaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(tw.openedu.mobile.R.layout.fragment_course_combined_info, container, false);

        announcementWebView = (EdxWebView) view.findViewById(tw.openedu.mobile.R.id.announcement_webview);
        ...
        notificationSettingRow = view.findViewById(tw.openedu.mobile.R.id.notificaton_setting_row);
        notificationSwitch = (Switch) view.findViewById(tw.openedu.mobile.R.id.notification_switch);
    ...
    private void loadAnnouncementData(EnrolledCoursesResponse enrollment) {
        ...
        ProgressBar progressBar = (ProgressBar) getView().findViewById(tw.openedu.mobile.R.id.loading_indicator);
    ...
    public void showEmptyAnnouncementMessage() {
        try {
            if (getView() != null) {
                getView().findViewById(tw.openedu.mobile.R.id.no_announcement_tv).setVisibility(View.VISIBLE);
            }
        } catch (Exception e) {
            logger.error(e);
        }
    ...
    private void hideEmptyAnnouncementMessage() {
        try {
            if (getView() != null) {
                getView().findViewById(tw.openedu.mobile.R.id.no_announcement_tv).setVisibility(View.GONE);
            }
        } catch (Exception e) {
            logger.error(e);
        }
